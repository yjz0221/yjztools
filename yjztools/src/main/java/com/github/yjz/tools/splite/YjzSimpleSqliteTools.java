package com.github.yjz.tools.splite;
 ...
 * SQLite封装工具类
 */
public class YjzSimpleSqliteTools {
    /**
     ...
     * SQLite工具类对象
     */
    private static volatile YjzSimpleSqliteTools sqLiteDbUtil;
    /**
     * 打印日志的TAG，用来调试
     */
    private final String TAG = "SimpleSqliteUtil";
     ...
     * @return 数据库管理工具类对象
     */
    public static YjzSimpleSqliteTools getSQLiteDbUtil() {
        if (sqLiteDbUtil != null) return sqLiteDbUtil;

        if (sqLiteDbUtil == null) {
            synchronized (YjzSimpleSqliteTools.class) {
                if (sqLiteDbUtil == null) {
                    sqLiteDbUtil = new YjzSimpleSqliteTools();
                }
            }
        }
    ...
    public <T> void createTable(final Class<T> c) {
        final String TABLE_NAME = JavaReflectTools.getClassName(c);
        final List<String> column = new ArrayList<>(Arrays.asList(JavaReflectTools.getAttributeNames(c)));
        final List<Class> type = new ArrayList<>(Arrays.asList(JavaReflectTools.getAttributeType(c)));
        ...
        if (!isTableExist(TABLE_NAME)) {//表不存在，创建表
            ...
            if (needModifyColumnType) {
                //1.查出原表数据
                List<T> list = YjzSimpleSqliteTools.getSQLiteDbUtil().query(c);
                //2.删除原表
                YjzSimpleSqliteTools.getSQLiteDbUtil().drop(TABLE_NAME);
                ...
                //4. 导入数据
                YjzSimpleSqliteTools.getSQLiteDbUtil().insert(list);
            }
        }
    }
    ...
    public <T> void drop(Class<T> c) {
        ...
        String TABLE_NAME = JavaReflectTools.getClassName(c);
    ...
    public <T> long insert(T t) {
        ...
        String TABLE_NAME = JavaReflectTools.getClassName(t.getClass());
     ...
     */
    public <T> List<Long> insert(List<T> lists) {
    ...
    private <T> T newInstance(Class<T> c, Cursor cursor) {
        try {
            ...
            T t = con.newInstance();
            Class[] types = JavaReflectTools.getAttributeType(c);
            String[] columns = JavaReflectTools.getAttributeNames(c);
    ...
    private <T> ContentValues getContentValues(T t) {
        List<Map<String, Object>> allInfo = JavaReflectTools.getAllFiledInfo(t);
